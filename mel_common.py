...
def pyatom(value):
  if pylist(value) or pydict(value):
    return False
...
def zeros_like(value, lib=None, dtype=None):
  lib = as_lib(lib, hint=value)
  return lib.zeros_like(value, dtype=_dtype(dtype))


def ones_like(value, lib=None, dtype=None):
  lib = as_lib(lib, hint=value)
  return lib.ones_like(value, dtype=_dtype(dtype))


# zeros = functools.partial(fill, value=0)
# ones = functools.partial(fill, value=1)


def zeros(value, lib=None, dtype=None):
  lib = as_lib(lib, hint=value)
  return lib.zeros(value, dtype=_dtype(dtype))


def ones(value, lib=None, dtype=None):
  lib = as_lib(lib, hint=value)
  return lib.ones(value, dtype=_dtype(dtype))


def append(a, b, axis=-1, lib=None):
  lib = as_lib(lib, hint=[a, b])
  a = cast(a, lib=lib)
  b = cast(b, lib=lib)
  return cat([a, b], axis=axis, lib=lib)


def cat(tensors, axis=-1, lib=None):
  lib = as_lib(lib, hint=tensors)
  if not pylist(tensors):
    return cast(tensors, lib=lib)
  elif pylist(tensors):
    tensors = [cast(x, lib=lib) for x in tensors]
  #tensors = cast(tensors, lib=lib)
  if lib == np:
    return np.concatenate(tensors, axis=axis)
  elif lib == tensorflow:
    return tensorflow.concat(tensors, axis=axis)
  else:
    assert lib is not None and lib == torch
    return torch.cat(tensors, dim=axis)


def transpose(value, indices):
  #if pylist(value):
  #  value = [val(x, eager=eager, session=session, deep=deep) for x in value]
  assert not pylist(value) and not pydict(value)
  if is_tf(value):
    return tensorflow.transpose(value, indices)
  if pyatom(value):
    value = np.array(value)
  if isinstance(value, np.ndarray):
    return value.transpose(indices)
  assert torch is not None
  value = cast(value, lib=torch)
  #torch.transpose(value, indices)
  return value.permute(indices)
...
def dtype(dtype, lib=None):
  if dtype is not None:
    return as_dtype(dtype_spec(dtype), lib=lib)
...
def swizzle(value, by=None):
  ...
  if isinstance(by, str):
    if len(by) > 0 and by[0] == '.':
      return swizzled(value, by=by[1:])
...
def cast(value, dtype=None, lib=None, hint=None):
  ...
  if pylist(value):
    value = stack(value, axis=-1, lib=lib)
    #return [cast(x, dtype=dtype, lib=lib, hint=hint) for x in value]
  elif pydict(value):
    # TODO: handle dict
    raise NotImplementedError()
  lib = as_lib(lib, hint=value)
  ...
  dtypes = pyflat(_dtype(dtype, lib=lib))
  if not functools.reduce(operator.eq, dtypes):
    raise ValueError("dtypes not equal for {}".format(value))
  dtype = dtypes[0]
  if lib == np:
    ...
    #if is_np(value):
    if not is_torch_type(value):
      value = cast(value, lib=np)
...
def _fill(shape, value, lib=None):
  if pylist(value):
...
def fill(shape, value, dtype=None, lib=None, hint=None):
  v = _fill(shape, value, lib=lib)
  v = cast(v, dtype=dtype, hint=hint)
  return v
