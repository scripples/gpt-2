...
def device_assignment(topology,
  ...
  missing_devices = get_missing_devices(topology)
...
def invert_topology(topology):
  ...
  tasks = np.full(list(topology.mesh_shape), -1, dtype=np.int32)
  devices = np.full(list(topology.mesh_shape), -1, dtype=np.int32)
...
def get_missing_devices(topology):
  topology_tasks, topology_devices = invert_topology(topology)
...
def spatial_partition(topology, num_cores_per_replica=1):
  num_cores = np.prod(topology.mesh_shape)
  if num_cores % num_cores_per_replica != 0:
    raise ValueError("Expected num_cores({}) to be divisible by num_cores_per_replica({})".format(num_cores, num_cores_per_replica))
  num_replicas = num_cores // num_cores_per_replica
